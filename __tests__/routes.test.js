...
describe('POST /auth/login', function() {
	...
	// ****** BUG #1 TEST FOR INCORRECT USERNAME/PASSWORD ******* //
	test('should NOT allow an incorrect username/password combination log in', async function() {
		const response = await request(app).post('/auth/login').send({
			username: 'u1',
			password: 'xxx'
		});
...
describe('PATCH /users/[username]', function() {
	...
	// *** TEST BUG #3: Sending tokens through query since
	// the body is restricted by json schema to first_name, last_name, phone, email *** //
	test('should patch data if admin', async function() {
		const response = await request(app)
			.patch('/users/u1')
			.query({ _token: tokens.u3 })
	...
	// *********** TEST BUG #4 *********** //
	test('should disallow patching username', async function() {
		const response = await request(app).patch('/users/u1').query({ _token: tokens.u1 }).send({ username: 'bad' });
		expect(response.statusCode).toBe(401);
	});

	test('should disallow patching _token', async function() {
		const response = await request(app).patch('/users/u1').query({ _token: tokens.u1 }).send({ _token: 'xxxx' });
		expect(response.statusCode).toBe(401);
	});

	test('should disallow patching admin', async function() {
		const response = await request(app).patch('/users/u1').query({ _token: tokens.u1 }).send({ admin: true });
		expect(response.statusCode).toBe(401);
	});
